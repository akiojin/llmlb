...
| `LLM_ROUTER_API_KEY` | - | ノード登録に必要なAPIキー（`X-API-Key`） |
...
│   └── Cargo.toml
├── node/                # C++ Node (ONNX Runtime ベース)
│   ├── src/
│   │   ├── main.cpp     # エントリーポイント
│   │   ├── api/         # OpenAI互換API
│   │   ├── core/        # 推論エンジン (ONNX Runtime / whisper.cpp)
│   │   └── models/      # モデル管理
│   ├── tests/           # TDDテスト
│   └── CMakeLists.txt
...
- **ルーター（Router）**: Linux / Windows 10以降 / macOS 12以降、Rust 1.70以降
- **ノード（Node）**: macOS 12以降 / Windows 10以降 / Linux、CMake + C++ツールチェーン（詳細は上の「Node (C++)」参照）
- **HFリポジトリに`.onnx`が無い場合のPython依存（ONNX export）**: `python3` が必要です。ルーターは `~/.llm-router/venv` を自動作成し、`scripts/requirements-export-hf-to-onnx.txt` をインストールします（失敗してもルーターは起動しますが、エクスポートは失敗します）。
...
**注意**: ノードはこのリポジトリからビルドし、`LLM_ROUTER_API_KEY` を使ってルーターへ登録します。外部の「LLM runtime」バイナリを別途ダウンロードする仕組みはありません。
...
3. main へのマージにより `.github/workflows/publish.yml` が動作し、Linux / macOS (x86_64, ARM64) / Windows 向けバイナリをビルド・検証し、GitHub Release に添付します。
   - この publish フェーズでは従来の `.tar.gz` / `.zip` アーカイブに加えて、`pkgbuild` で作成した macOS 向け `llm-router-<platform>.pkg` / `llm-node-<platform>.pkg` と、WiX Toolset で作成した Windows 向け `llm-router-<platform>.msi` / `llm-node-<platform>.msi` を個別に生成・添付します。既存のリリース資産は削除せず、そのまま維持します。
...
### Hugging Face登録 (ONNX優先)

- オプション環境変数: レートリミット回避に `HF_TOKEN`、社内ミラー利用時は `HF_BASE_URL` を指定。
- Web:
  - ダッシュボード → モデル管理 → 「Register Model (HF URL)」
  - `https://huggingface.co/org/repo/...` またはプレーンな `org/repo`（任意で `org/repo@revision`）を貼り付け。
  - `.onnx` があればそれを優先し、無ければルーターがエクスポートジョブを投入して `model.onnx` + `manifest.json` を生成します（デフォルト: `scripts/export_hf_to_onnx.py`。必要なら `LLM_CONVERT_SCRIPT` で差し替え）。
  - ノードは `manifest.json` に含まれる `registry_url` を使って `/api/models/registry/...` から一括取得します。
...
| メソッド | パス | 機能 | 認証 |
|---------|------|------|------|
| POST | `/api/nodes` | ノード登録（GPU必須） | APIキー |
...
| GET | `/api/models/registry/*path` | モデルレジストリアーティファクト配信 | 不要 |
...
| メソッド | パス | 機能 | 認証 |
|---------|------|------|------|
| GET | `/api/dashboard/logs/router` | ルーターログ | 不要 |
