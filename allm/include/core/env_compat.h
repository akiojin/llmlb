#pragma once
/// @file env_compat.h
/// @brief Environment variable compatibility layer for ALLM_* and ALLM_* migration
///
/// This header provides functions to read environment variables with backwards
/// compatibility. New ALLM_* variables take precedence over legacy ALLM_* variables.
...
/// Get an environment variable with ALLM_*/ALLM_* fallback.
/// @param allm_name New ALLM_* variable name (without prefix)
/// @param legacy_name Legacy ALLM_* variable name (without prefix), or nullptr to use allm_name
...
inline const char* get(const char* allm_name, const char* legacy_name = nullptr) {
    ...
    // Fall back to ALLM_* (legacy)
    std::string legacy_var = std::string("ALLM_") + (legacy_name ? legacy_name : allm_name);
    return std::getenv(legacy_var.c_str());
}

/// Check if an environment variable is set (ALLM_*/ALLM_* with fallback).
inline bool is_set(const char* allm_name, const char* legacy_name = nullptr) {
    return get(allm_name, legacy_name) != nullptr;
}

/// Get an environment variable as a boolean (ALLM_*/ALLM_* with fallback).
...
/// Get an environment variable as an integer (ALLM_*/ALLM_* with fallback).
