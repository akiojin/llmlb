...
void NodeEndpoints::registerRoutes(httplib::Server& server) {
    ...
    server.Get("/api/node/stats", [this](const httplib::Request&, httplib::Response& res) {
    ...
    server.Post("/log/level", [](const httplib::Request& req, httplib::Response& res) {
        ...
        const std::string level_str = j["level"].get<std::string>();
        ...
        if (level_str == "trace") level = spdlog::level::trace;
            ...
            res.status = 400;
            res.set_content(R"({"error":"invalid level"})", "application/json");
            return;
        }
...
void NodeEndpoints::setModelSync(std::shared_ptr<ModelSync> sync) {
    model_sync_ = std::move(sync);
}

void NodeEndpoints::setRouterClient(std::shared_ptr<RouterClient> client) {
    router_client_ = std::move(client);
}
