...
TEST(ModelRegistryTest, IsCompatibleReturnsTrueForLoadedModels) {
    ModelRegistry registry;
    registry.setModels({"gpt-oss-7b", "llama-3.1-8b"});

    // 登録済みモデルは現在のバックエンドと互換性あり
    EXPECT_TRUE(registry.isCompatible("gpt-oss-7b", GpuBackend::kMetal));
    EXPECT_TRUE(registry.isCompatible("llama-3.1-8b", GpuBackend::kCuda));
    EXPECT_TRUE(registry.isCompatible("gpt-oss-7b", GpuBackend::kRocm));
    EXPECT_TRUE(registry.isCompatible("gpt-oss-7b", GpuBackend::kCpu));
}
...
TEST(ModelRegistryTest, HasModelReturnsTrueForExistingModel) {
    ...
    EXPECT_TRUE(registry.hasModel("gpt-oss-7b"));
