...
TEST(OnnxLlmManagerTest, LoadsExistingModel) {
    TempModelFile tmp;
    fs::path model = tmp.base / "model.gguf";
    fs::create_directories(model.parent_path());
    // Note: This creates an invalid model file (just the magic bytes)
    // ONNX Runtime will fail to load it as a valid ONNX model
    std::ofstream(model) << "GGUF";

    OnnxLlmManager mgr(tmp.base.string());
    // Invalid file will fail to load in ONNX Runtime.
...
TEST(OnnxLlmManagerTest, FailsOnMissingModel) {
    TempModelFile tmp;
    OnnxLlmManager mgr(tmp.base.string());
...
TEST(OnnxLlmManagerTest, RejectsUnsupportedExtension) {
    TempModelFile tmp;
...
TEST(OnnxLlmManagerTest, UnloadReducesMemory) {
    TempModelFile tmp;
    fs::path model = tmp.base / "m.gguf";
    fs::create_directories(model.parent_path());
    // Invalid file - ONNX Runtime will reject it
    std::ofstream(model) << "GGUF";
    OnnxLlmManager mgr(tmp.base.string());
...
TEST(OnnxLlmManagerTest, AcceptsRuntimeBlobFormat) {
    TempModelFile tmp;
...
TEST(OnnxLlmManagerTest, RejectsInvalidBlobFormat) {
    TempModelFile tmp;
...
TEST(OnnxLlmManagerTest, RejectsBlobWithInvalidHex) {
    TempModelFile tmp;
...
TEST(OnnxLlmManagerTest, RejectsTooShortBlobName) {
    TempModelFile tmp;
    // Too short blob name
