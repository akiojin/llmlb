...
bool load_model_config(
    ...
    if (!file.is_open()) {
        error = "Failed to open config.json: " + config_path.string();
        return false;
    }
    ...
    while (p < end) {
        ...
        if (key == "num_hidden_layers" || key == "n_layer") {
            model.n_layers = json_parser::parse_int(p, end);
        } else if (key == "num_attention_heads" || key == "n_head") {
            model.n_heads = json_parser::parse_int(p, end);
        } else if (key == "hidden_size" || key == "n_embd") {
            model.hidden_size = json_parser::parse_int(p, end);
        } else if (key == "vocab_size") {
            model.vocab_size = json_parser::parse_int(p, end);
        } else if (key == "max_position_embeddings" || key == "n_positions") {
            model.max_context = json_parser::parse_int(p, end);
        } else {
            json_parser::skip_value(p, end);
        }
