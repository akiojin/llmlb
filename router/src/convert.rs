...
async fn notify_nodes_of_new_model(model_name: &str) {
    ...
    for node in online_nodes {
        ...
        let node_port = node
            .node_api_port
            .unwrap_or_else(|| node.runtime_port.saturating_add(1));
        let node_addr = format!("http://{}:{}", node.ip_address, node_port);
        ...
        match result {
            Ok(()) => {
                let meta = tokio::fs::metadata(target)
                    .await
                    .map_err(|e| RouterError::Internal(format!("ONNX output is missing: {}", e)))?;
                if meta.len() == 0 {
                    return Err(RouterError::Internal(
                        "Conversion produced empty ONNX file (0 bytes)".into(),
                    ));
                }
