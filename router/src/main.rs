...
impl ServerConfig {
    fn from_env() -> Self {
        let host = get_env_with_fallback_or("LLMLB_HOST", "ROUTER_HOST", "0.0.0.0");
        let port = get_env_with_fallback_parse("LLMLB_PORT", "ROUTER_PORT", 32768);
        Self { host, port }
    }
...
async fn run_server(config: ServerConfig) {
    ...
    let database_url = llmlb::config::get_env_with_fallback("LLMLB_DATABASE_URL", "DATABASE_URL")
        .unwrap_or_else(|| {
            ...
            format!("sqlite:{}/.llmlb/router.db", home)
        });
