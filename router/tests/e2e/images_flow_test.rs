...
async fn register_image_node(router: &support::http::TestServer, node: &support::http::TestServer) {
    let response = support::router::register_node_with_runtimes(
        router.addr(),
        node.addr(),
        vec!["stable_diffusion"],
    )
    .await
...
async fn e2e_images_generations_returns_image() {
    ...
    register_image_node(&router, &node).await;
...
async fn e2e_images_edits_returns_image() {
    ...
    let response = Client::new()
        .post(format!("http://{}/v1/images/edits", router.addr()))
        .header("x-api-key", "sk_debug")
        .multipart(form)
        .send()
        .await
