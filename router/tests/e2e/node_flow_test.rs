...
async fn fetch_node_status(app: &Router, node_id: Uuid) -> String {
    let response = app
        .clone()
        .oneshot(
            Request::builder()
...
async fn test_complete_node_flow() {
    ...
    let pending_status = fetch_node_status(&app, node_id).await;
    assert_eq!(
...
async fn test_node_token_persistence() {
    ...
    let status = fetch_node_status(&app, node_id).await;
    assert_eq!(
        status, "pending",
