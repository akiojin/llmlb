...
async fn test_integration_register_safetensors_requires_metadata() {
    ...
    let (router, db_pool) = support::router::spawn_test_router_with_db().await;
    let admin_key = support::router::create_test_api_key(router.addr(), &db_pool).await;
    ...
    let response = Client::new()
        .post(format!("http://{}/v0/models/register", router.addr()))
