...
use crate::support::{
    ...
    router::{register_responses_endpoint, spawn_test_router},
};
use axum::{
    extract::State,
    http::StatusCode,
...
async fn res001_responses_passthrough_preserves_request_body() {
    ...
    let _ = register_responses_endpoint(router.addr(), node_stub.addr(), "integration-model")
        .await
...
async fn res001_responses_passthrough_with_tools() {
    ...
    let _ = register_responses_endpoint(router.addr(), node_stub.addr(), "tool-model")
        .await
        .expect("register endpoint must succeed");
...
async fn res003_non_supporting_backend_returns_501() {
    ...
    // エンドポイントを登録
    let _ = register_responses_endpoint(router.addr(), node_stub.addr(), "chat-only-model")
        .await
        .expect("register endpoint must succeed");
...
async fn models_api_includes_supported_apis_field() {
    ...
    let _ = register_responses_endpoint(router.addr(), node_stub.addr(), "responses-capable-model")
        .await
        .expect("register endpoint must succeed");
