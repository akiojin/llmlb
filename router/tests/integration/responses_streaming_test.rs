...
async fn responses_streaming_passthrough_events() {
    ...
    let _ = register_responses_endpoint(router.addr(), node_stub.addr(), "streaming-model")
        .await
...
async fn responses_streaming_events_preserve_order() {
    ...
    // エンドポイントを登録（Responses API対応検出付き）
    let _ = register_responses_endpoint(router.addr(), node_stub.addr(), "order-test-model")
        .await
        .expect("register endpoint must succeed");
...
async fn responses_streaming_collects_full_text() {
    ...
    // エンドポイントを登録（Responses API対応検出付き）
    let _ = register_responses_endpoint(router.addr(), node_stub.addr(), "text-collect-model")
        .await
        .expect("register endpoint must succeed");
