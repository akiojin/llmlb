...
本機能は、**ロードバランサー側でリクエストキューを管理**し、各ノードには**単発リクエストのみ**を送信することで、
システム全体の安定性とユーザー体験を維持する。ノードは推論処理に専念し、キュー管理の複雑さはロードバランサーに集約される。

## ユーザーシナリオ＆テスト

### ユーザーストーリー1 - 単一ノード環境での連続リクエスト処理 (優先度: P1)

ユーザーが単一ノード環境でLLM Load Balancerを使用している場合、
...
**独立テスト**: ノードが推論中に新しいリクエストを送信し、そのリクエストがロードバランサーのキューで待機することで価値を提供する。
...
2. **前提** すべてのノードが推論中かつロードバランサーのキューが満杯、**実行** 新しいリクエストを送信する、
...
- ロードバランサーのキュー内のリクエストがタイムアウト前にノードがアイドルになった場合、リクエストは正常に処理される
...
- **FR-001**: ロードバランサーはグローバルなリクエストキューを維持する必要がある（エンドポイント側ではなくロードバランサー側で管理）
...
- **FR-010**: ノードは推論中に新規リクエストを受信した場合、即座にHTTP 429を返す必要がある（エンドポイント側キューイング禁止）
...
- **リクエストキュー**: ロードバランサー側で管理するグローバルな待機列。最大サイズ、現在の待機数を持つ
...
- **キュー満杯の判定**: アイドル判定の揺らぎに依存せず、in-flight の存在が確実に反映されること
- **テスト安定性**: キュー満杯テストは「先行リクエストが負荷計測に反映された後」に次リクエストを送る
