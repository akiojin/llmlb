...
本機能は、推論中のノードに対するリクエストを適切にキューイング・待機・拒否することで、
システム全体の安定性とユーザー体験を維持する。
...
503エラーと適切なメッセージが返されることで価値を提供する。

**受け入れシナリオ**:

1. **前提** キューが最大サイズ（デフォルト: 100）に達している、**実行** 新しいリクエストを送信する、
   **結果** HTTP 503 Service Unavailable と「キューが満杯です」メッセージが返される
...
### ユーザーストーリー4 - ノードごとの同時推論制限 (優先度: P2)

管理者として、ノードごとに同時に処理できる推論リクエスト数を制限し、
ノードの過負荷を防止できることを期待する。

**この優先度の理由**: GPUメモリの効率的な使用とシステム安定性の維持に重要。

**独立テスト**: ノードの同時推論数を2に設定し、3つ目のリクエストがキューで待機することで価値を提供する。

**受け入れシナリオ**:

1. **前提** ノードの同時推論制限が2に設定、**実行** 同時に3つのリクエストを送信する、
   **結果** 2つは即座に処理開始、1つはキューで待機する
2. **前提** ノードが同時推論制限に達している、**実行** ダッシュボードを確認する、
   **結果** 「処理中: 2/2」「キュー待機: 1」のように状態が表示される

---

### ユーザーストーリー5 - 複数ノード環境での効率的な分散 (優先度: P3)

複数ノード環境では、1つのノードのキューが満杯でも、
他の空いているノードにリクエストが自動的にルーティングされることを期待する。

**この優先度の理由**: 複数ノード環境での効率的なリソース活用。

**独立テスト**: 2台のノードのうち1台のキューが満杯の状態で、
リクエストが空いているノードにルーティングされることで価値を提供する。

**受け入れシナリオ**:

1. **前提** 2台のノードが登録され、ノードAのキューが満杯、ノードBは空き、
   **実行** 新しいリクエストを送信する、**結果** リクエストはノードBにルーティングされる
2. **前提** すべてのノードのキューが満杯、**実行** 新しいリクエストを送信する、
   **結果** HTTP 503とすべてのノードが過負荷であることを示すメッセージが返される

---

### エッジケース

- キュー内のリクエストがタイムアウト前にノードが復旧した場合、リクエストは正常に処理される
- ノードがオフラインになった場合、そのノードのキュー内リクエストは他ノードに再ルーティングされるか、
  エラーを返す
...
- **FR-001**: システムは各ノードに対してリクエストキューを維持する必要がある
- **FR-002**: システムはキューの最大サイズを設定可能にする必要がある（デフォルト: 100リクエスト）
- **FR-003**: システムはキュー待機タイムアウトを設定可能にする必要がある（デフォルト: 60秒）
- **FR-004**: システムはノードごとの同時推論数を制限可能にする必要がある（デフォルト: 1）
- **FR-005**: キューが満杯の場合、システムはHTTP 503とRetry-Afterヘッダーを返す必要がある
- **FR-006**: キュー待機タイムアウトの場合、システムはHTTP 504を返す必要がある
- **FR-007**: システムはキュー状態（待機数、処理中数）をダッシュボードで表示する必要がある
- **FR-008**: 複数ノード環境では、キューに空きのあるノードを優先的に選択する必要がある
- **FR-009**: クライアント切断時、対応するキュー内リクエストはキャンセルされる必要がある

### 主要エンティティ

- **リクエストキュー**: 各ノードに紐づくリクエストの待機列。最大サイズ、現在の待機数を持つ
- **キューエントリ**: キュー内の個別リクエスト。到着時刻、タイムアウト期限、クライアント接続状態を持つ
...
2. キュー満杯時、追加リクエストに対して1秒以内にHTTP 503が返される
3. キュー待機タイムアウト時、設定値の誤差5秒以内にHTTP 504が返される
4. 複数ノード環境で、キューに空きのあるノードへのルーティング成功率が95%以上
