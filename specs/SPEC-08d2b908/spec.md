...
## スコープ
- ロードバランサー側の登録フロー（URL入力・重複・削除・一覧）
...
- ロードバランサー側のモデルバイナリキャッシュ

## 原則
- ロードバランサーは**登録メタデータとマニフェストのみ**を保持する
- **モデルキャッシュはNode主導**（GPU差分に応じて必要アーティファクトを選択・取得）
- ロードバランサー側の**変換パイプラインは廃止**（登録時に変換しない）
- `config.json` や `tokenizer.json` などHF由来メタデータを正とする

## 決定事項（共有用サマリ）
- **ロードバランサーは形式選択を行わない**。Nodeがruntime/GPU要件に応じて選択する。
- **ダウンロード主体**: Node が外部ソース（HF等）から直接ダウンロードし、
  ロードバランサーはバイナリを保持しない。
- **マニフェスト**: ロードバランサーはHFのファイル一覧をマニフェストとして提供する。
