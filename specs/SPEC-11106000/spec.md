...
**独立テスト**: 非GGUF（safetensors）URLをRegister→ロードバランサーは変換しない→NodeがHFから直接取得→推論成功。
...
- **FR-002**: ロードバランサーは**変換もバイナリダウンロードもしない**。登録はメタデータ保存に限定する。
...
- 変換パイプラインは廃止し、ロードバランサーはバイナリを生成しない。

## 前提条件
- ロードバランサーとノード間の既存配布・登録フローが動作していること。
- 管理者はHF公開モデルへ到達できること。

## 依存関係
- SPEC-dcaeaec4（LLM-Load Balancer独自モデルストレージ）- モデル配布とストレージの仕組み
