...
- [ ] T001 `node/third_party/` に whisper.cpp をサブモジュールとして追加
- [ ] T002 `node/CMakeLists.txt` に whisper.cpp ビルド設定を追加
- [ ] T003 [P] `node/CMakeLists.txt` に ONNX Runtime 依存関係を追加
- [ ] T004 [P] `common/src/types.rs` に AudioFormat enum を追加
- [ ] T005 `common/src/types.rs` に RuntimeType enum を追加

## Phase 3.2: テストファースト (TDD) - 実装前に失敗必須

### 3.2.1 Contract Tests (API契約テスト)

- [ ] T006 [P] `router/tests/audio_transcriptions_contract_test.rs` に POST /v1/audio/transcriptions の契約テスト
- [ ] T007 [P] `router/tests/audio_speech_contract_test.rs` に POST /v1/audio/speech の契約テスト

### 3.2.2 型定義テスト

- [ ] T008 [P] `common/src/types.rs` の tests モジュールに ModelType 拡張のテスト追加
- [ ] T009 [P] `common/src/types.rs` の tests モジュールに RuntimeType のテスト追加
- [ ] T010 [P] `common/src/protocol.rs` の tests モジュールに RequestType 拡張のテスト追加
- [ ] T011 [P] `common/src/protocol.rs` の tests モジュールに TranscriptionRequest/Response のテスト追加
- [ ] T012 [P] `common/src/protocol.rs` の tests モジュールに SpeechRequest のテスト追加

### 3.2.3 Integration Tests (ユーザーストーリー検証)

- [ ] T013 [P] `router/tests/integration/audio_asr_test.rs` にストーリー1: 音声認識テスト (日本語WAV, 英語MP3, タイムスタンプ)
- [ ] T014 [P] `router/tests/integration/audio_tts_test.rs` にストーリー2: 音声合成テスト (日本語, voice選択)
- [ ] T015 [P] `router/tests/integration/audio_routing_test.rs` にストーリー3: 複数ノード分散テスト
- [ ] T016 [P] `router/tests/integration/audio_model_management_test.rs` にストーリー4: モデル登録管理テスト
...
- [ ] T022 [P] `common/src/protocol.rs` に SpeechRequest 構造体を追加
- [ ] T023 `common/src/types.rs` の Node 構造体に loaded_asr_models, loaded_tts_models, supported_runtimes を追加
- [ ] T024 `common/src/protocol.rs` の HealthCheckRequest に loaded_asr_models, loaded_tts_models, supported_runtimes を追加

## Phase 3.4: Router側API実装

- [ ] T025 `router/src/api/audio.rs` を新規作成 (モジュール構造のみ)
- [ ] T026 `router/src/api/mod.rs` に audio モジュールを追加
- [ ] T027 `router/src/api/audio.rs` に POST /v1/audio/transcriptions ハンドラを実装
- [ ] T028 `router/src/api/audio.rs` に POST /v1/audio/speech ハンドラを実装
- [ ] T029 `router/src/api/audio.rs` に multipart/form-data パーサーを実装
- [ ] T030 `router/src/api/proxy.rs` に RuntimeType 別ノード選択ロジックを追加
- [ ] T031 `router/src/registry/models.rs` の ModelInfo に runtime_type フィールドを追加
- [ ] T032 `router/src/lib.rs` に audio API ルートを登録

## Phase 3.5: Node側 whisper.cpp 統合 (ASR)

- [ ] T033 `node/include/core/whisper_manager.h` を新規作成 (WhisperManager クラス定義)
- [ ] T034 `node/src/core/whisper_manager.cpp` を新規作成 (whisper.cpp 初期化・モデルロード)
- [ ] T035 `node/src/core/whisper_manager.cpp` に transcribe() メソッドを実装
- [ ] T036 `node/src/core/whisper_manager.cpp` に音声デコード処理を実装 (WAV/MP3/FLAC)
- [ ] T037 `node/src/api/audio_endpoints.cpp` を新規作成 (POST /v1/audio/transcriptions ハンドラ)
- [ ] T038 `node/CMakeLists.txt` に whisper_manager をビルドターゲットに追加

## Phase 3.6: Node側 ONNX Runtime 統合 (TTS)

- [ ] T039 `node/include/core/onnx_tts_manager.h` を新規作成 (OnnxTtsManager クラス定義)
- [ ] T040 `node/src/core/onnx_tts_manager.cpp` を新規作成 (ONNX Runtime 初期化・モデルロード)
- [ ] T041 `node/src/core/onnx_tts_manager.cpp` に synthesize() メソッドを実装
- [ ] T042 `node/src/core/onnx_tts_manager.cpp` に音声エンコード処理を実装 (MP3/WAV)
- [ ] T043 `node/src/api/audio_endpoints.cpp` に POST /v1/audio/speech ハンドラを追加
- [ ] T044 `node/CMakeLists.txt` に onnx_tts_manager をビルドターゲットに追加
...
- [ ] T048 `router/src/api/audio.rs` にエラーハンドリング (OpenAI API形式) を追加
- [ ] T049 `router/src/api/audio.rs` にリクエストログ出力を追加
