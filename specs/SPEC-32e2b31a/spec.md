...
2. ~~**前提** ノードが起動している、**実行** 設定ウィンドウでルーターのURL（例: `http://192.168.1.10:32768`）を入力して「接続」をクリックする、**結果** ノードがルーターに登録され、接続状態が「接続済み」と表示される~~ → **変更**: 環境変数`LLMLB_URL`で設定、WebUIで状態確認
...
**独立テスト**: ルーターのエンドポイント（例: `http://router:32768/v1/chat/completions`）に対してOpenAI互換API形式のリクエストを送信し、正常にレスポンスが返ることで価値を提供する。

**受け入れシナリオ**:

1. **前提** 2台のノードが登録されている、**実行** ルーターのAPI（`POST /v1/chat/completions`）にチャットリクエストを送信する、**結果** いずれかのノードが処理し、LLMからのレスポンスが返される
...
1. **前提** ルーターが起動している、**実行** ブラウザで`http://router:32768/dashboard`にアクセスする、**結果** ダッシュボードが表示され、登録済みノードの一覧が確認できる
...
- **FR-004a**: ルーターはOpenAI互換APIエンドポイント（`/v1/chat/completions`, `/v1/completions`）を提供する必要がある
...
- ポート要件: ルーターはデフォルトでポート32768、LLM runtimeはデフォルトでポート32768を使用
...
13. **リリース配布形式の整合性**: GitHubリリースに添付される配布物は、Unix系プラットフォーム向けを`.tar.gz`、Windows向けを`.zip`で提供し、それぞれにRouter/Nodeバイナリと最低限 `README.md` / `README.ja.md` / `LICENSE` が同梱されている（追加ドキュメントを含める場合はリリースノートで明示する）
