...
**独立テスト**: ルーターのエンドポイント（例: `http://router:8080/v1/chat/completions`）に対してOpenAI互換API形式のリクエストを送信し、正常にレスポンスが返ることで価値を提供する。
...
1. **前提** ルーターが起動している、**実行** ブラウザで`http://router:8080/dashboard`にアクセスする、**結果** ダッシュボードが表示され、登録済みノードの一覧が確認できる
