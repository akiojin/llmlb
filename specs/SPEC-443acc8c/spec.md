...
> **廃止理由**:
>
> - プッシュ型（ノード→ルーター）からプル型（ルーター→エンドポイント）に変更
...
> **新方式**:
>
> - ルーター側から各エンドポイントへ定期的にヘルスチェックを実行（プル型）
...
- 送信間隔: 30秒ごと（Node側設定。例: `LLM_NODE_HEARTBEAT_SECS`）
...
ルーターはノードからのヘルスチェック受信エンドポイントを提供する。
...
- ハートビート送信間隔: 30秒（Node側、`LLM_NODE_HEARTBEAT_SECS`で設定可能）（FR-013で明記）
...
- プッシュ型: ノード→ルーターへのハートビート送信（ルーターからのポーリングなし）
