...
**作成日**: 2025-12-20
**ステータス**: ✅ 実装完了（6/6タスク完了）
**入力**: ユーザー説明: "モデルメタデータ（models.json）をSQLiteに統合し、認証システム（lb.db）と同じDBで管理する。タグ検索やソースフィルタリングを高速化する。"
...
1. **前提** 100件以上のモデルが登録されている、**実行** ダッシュボードのモデル一覧を開く、
   **結果** 1秒以内に一覧が表示される
2. **前提** ロードバランサーが起動している、**実行** モデル一覧APIを呼び出す、
   **結果** モデル情報が正しく返される
...
管理者として、モデルのソース（HuggingFace safetensors、HuggingFace GGUF、HuggingFace ONNX、
...
**独立テスト**: 特定のソース（例: "hf_safetensors"）でフィルタし、
該当するモデルのみが返されることを確認。

**受け入れシナリオ**:

1. **前提** 複数ソースのモデルが登録されている、**実行** ソース"hf_safetensors"でフィルタ、
   **結果** HuggingFace safetensorsソースのモデルのみが表示される

---

### ユーザーストーリー4 - 既存データの自動移行 (優先度: P1)

システム管理者として、ロードバランサーのアップデート後に既存の`models.json`データが
...
**独立テスト**: 既存の`models.json`がある状態でロードバランサーを起動し、
データが正しく移行されていることを確認。

**受け入れシナリオ**:

1. **前提** 既存の`models.json`にモデルデータがある、**実行** ロードバランサーを起動、
   **結果** モデルデータが新形式に移行され、元ファイルは`.migrated`にリネームされる
2. **前提** 移行済みの状態、**実行** ロードバランサーを再起動、
...
- 移行中にロードバランサーがクラッシュした場合、データは失われるか？
...
- **FR-004**: システムはソースでのモデルフィルタリングをサポートする必要がある（例: `hf_safetensors`, `hf_gguf`, `hf_onnx`）
- **FR-005**: システムは既存の`models.json`からの自動移行をサポートする必要がある
- **FR-006**: システムは移行後も既存のモデル管理APIと互換性を維持する必要がある
- **FR-007**: システムはロードバランサー再起動後もモデルデータを永続化する必要がある
...
- 既存の認証システム（`lb.db`）と同じSQLiteデータベースを使用する
...
- ロードバランサーの認証システム（SQLite）が既に実装済み
...
- 保存先: 認証システムと同じDB（lb.db）（FR-001で明記）
...
5. ロードバランサー再起動後もモデルデータが永続化されている
