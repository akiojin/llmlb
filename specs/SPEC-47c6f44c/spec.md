...
- 品質チェック項目: tasks-check, cargo test, cargo fmt/clippy, commitlint, markdownlint（FR-002~FR-006で明記）
- マージ方式: MERGE方式（履歴保持）（FR-010で明記）
- 自動マージ条件: mergeable=MERGEABLEかつDraftでないPR（FR-008, FR-009で明記）
- 冪等性: 既存の自動マージ設定はスキップ（FR-011で明記）
- ドラフト除外: Ready for reviewイベント後に再試行（FR-012で明記）

**ワークフロー構成**:

- 品質チェックワークフロー: 5ジョブ並列実行
- 自動マージワークフロー: `pull_request_target`イベントで起動、GraphQL APIで設定
