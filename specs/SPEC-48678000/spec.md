...
**更新日**: 2026-01-25
**ステータス**: 改定中（自動認識への移行）
**入力**: ユーザー説明: "モデル自動解決機能: ノード側でモデルが存在しない場合、HuggingFaceから直接取得し、config.jsonからアーキテクチャを自動検出する。"
...
- **FR-002**: ~~システムはローカルにモデルがない場合、`supported_models.json`の定義を参照し、外部ソース（Hugging Face等）からダウンロードする必要がある~~ **改定**: システムはローカルにモデルがない場合、HuggingFace URLから直接ダウンロードする必要がある
