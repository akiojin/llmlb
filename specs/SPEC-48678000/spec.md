...
- **FR-003**: システムは共有パスがアクセス不可の場合、ルーターの `/v1/models/blob/:model_name` エンドポイント経由でモデルをダウンロードする必要がある
...
- **ルーターAPI**: モデルファイルをHTTP経由で配信するルーターのエンドポイント（`/v1/models/blob/:model_name`）
...
- 解決優先順位: ローカル → 共有パス → ルーターAPI（FR-001〜FR-003で明記）
- 共有パス動作: 直接参照、コピーしない（FR-002で明記）
- 禁止事項: HuggingFace直接ダウンロード禁止（FR-006で明記）
- 削除対象: auto_repair機能の完全削除（FR-007で明記）
- エラー応答時間: 1秒以内（成功基準3で明記）

**実装時の判断に委ねる事項**:

- ルーターAPI経由ダウンロード時のタイムアウト値（推奨: 5分）
