...
**作成日**: 2025-12-06
**ステータス**: 実装完了
**入力**: ユーザー説明: "モデル自動解決機能: ノード側でモデルが存在しない場合、supported_models.jsonに定義されたモデルを外部ソース（Hugging Face等）から直接取得する。"
...
3. **前提** モデルのダウンロード中、**実行** ルーターのダッシュボードを開く、**結果** ノードの同期状態（同期中/完了/失敗）と進捗が表示される

---

### ユーザーストーリー2 - 外部ソース直取得の必須化 (優先度: P2)

ノードオペレーターとして、ノードが外部ソース（Hugging Face等）から直接モデルをダウンロードできることを必須条件としたい。ルーターはメタデータを提供するだけで、モデルバイナリは保持しない。

**この優先度の理由**: ルーターのキャッシュ/変換に依存せず、ノードが自律的に取得・保持する運用を保証するため。

**独立テスト**: ルーターがバイナリを保持していない状態でも、ノードが外部ソースから直接ダウンロードして推論できることを確認する。
...
- **FR-009**: ノードは同期状態（idle/running/success/failed）と進捗（モデル名・ファイル名・ダウンロード量）をルーターへ通知する必要がある
- **FR-010**: ルーターは最新の同期状態を保持し、APIおよびダッシュボードで可視化できる必要がある
...
- **ルーターAPI**: メタデータ/モデル一覧の提供に使用（バイナリ配信は必須ではない）
...
- **Node が外部ソースから直接取得**し、ルーターはバイナリを保持しない
...
1. `supported_models.json`に定義されたモデルがローカルにない場合でも、外部ソースから直接取得され、推論リクエストが成功する
