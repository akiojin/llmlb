...
- [ ] C-6: finish_request()にトークン集計呼び出し追加
  - 依存: C-2, C-3
- [ ] C-7: NodeLoadStateトークン累積ロジック実装
  - 依存: C-6

### 永続化実装

- [ ] C-8: マイグレーション適用（SQLiteスキーマ変更）
  - 依存: S-2
- [ ] C-9: insert_record()でトークン値バインド追加
  - 依存: C-1, C-8, T-6
- [ ] C-10: トークン集計クエリ実装
  - 依存: C-8, T-7
...
- [ ] I-3: /api/dashboard/stats/tokens エンドポイント実装
  - 依存: C-10, T-10
- [ ] `[P]` I-4: /api/dashboard/stats/tokens/daily エンドポイント実装
  - 依存: I-3
- [ ] `[P]` I-5: /api/dashboard/stats/tokens/monthly エンドポイント実装
  - 依存: I-3
