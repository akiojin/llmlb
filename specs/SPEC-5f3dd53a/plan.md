...
- 責務分離: Load Balancerはマニフェスト提示、NodeがDLL/アーティファクト解決を担当。
...
- Node: `node/engines/gptoss/cuda/` と `node/engines/nemotron/cuda/` に CUDA DLL のソースを配置し、CMake で `gptoss_cuda.dll` / `nemotron_cuda.dll` を生成
