...
- **TTS拡張パラメータ**: `tone`/`speed`/`pitch`等の音声制御パラメータを追加（OpenAI API拡張）
...
- **フォーマット変換**: エンドポイント側で実施（ロードバランサーは変換しない。MP3→WAV等はNodeがFFmpeg等でネイティブ変換）
...
- **TTS Voiceマッピング**: モデル単位マッピング方式（各voice="alloy", "echo"等に個別のモデルファイルが対応）
- **ASRメタデータ**: `supported_models.json`でのASRモデル定義にメタデータ必須:
  - 対応言語（サポート言語一覧）
  - モデルサイズ（tiny/base/small/medium/large/large-v2/large-v3）
  - 最大音声長（秒単位）
- **ASRストリーミング中断**: クライアント切断時は即座に処理を破棄（部分結果は返却しない）
...
- **FR-015**: システムはTTSで拡張パラメータ（`tone`/`speed`/`pitch`等）による音声制御をサポートする必要がある
...
- **FR-019**: 音声ファイルのフォーマット変換（MP3→WAV等）はエンドポイント側で実施し、ロードバランサーは変換を行わない
