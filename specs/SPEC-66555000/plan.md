...
- すべての機能をライブラリとして? ✅ router/src/以下にモジュール化
- ライブラリリスト:
...
- `Endpoint`: エンドポイント情報（name, url, status, api_key等）
- `EndpointModel`: エンドポイントで利用可能なモデル情報
...
```text
# エンドポイント管理API（認証必須）
POST   /v0/endpoints              # 登録
GET    /v0/endpoints              # 一覧
GET    /v0/endpoints/:id          # 詳細
PUT    /v0/endpoints/:id          # 更新
