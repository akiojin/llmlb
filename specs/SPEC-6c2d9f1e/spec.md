...
### ユーザーストーリー1 - モデル登録（Nodeキャッシュ優先） (P1)
運用管理者として、モデル登録はメタデータのみ行い、Node側のローカルキャッシュがあれば再ダウンロードせず即時利用できることを期待する。破損/0Bのキャッシュは無効として再取得されることが必要。

**独立テスト**:
- 既存キャッシュ（正常サイズ）→ Nodeは再ダウンロード無しで利用
- 0Bキャッシュ → Nodeが再ダウンロードし、ready が true になる

### ユーザーストーリー2 - モデル指定チャット (P1)
利用者として、/v1/chat/completions で指定したモデルが実際に選択され、正常にチャットが返ることを期待する。

**独立テスト**:
- 指定モデル名で Chat API を実行し、エンドポイント側で当該モデルがロードされる
- 未登録モデルは 404 / 503 を返す
...
### ユーザーストーリー5 - モデル削除 (P1)
運用管理者として、モデルを削除すると /v1/models から即時消え、ロードバランサーの登録情報とノードのローカルキャッシュが削除されることを期待する（ロードバランサーはバイナリを保持しない）。

**独立テスト**:
- DELETE /v1/models/:name でモデルを削除後に /v1/models から消える
- エンドポイント側のモデルも同期で削除される

## 要件 *(必須)*

### 機能要件
- **FR-001**: 登録はメタデータのみ行い、Nodeはローカルキャッシュが存在しサイズが 0 でない場合は再ダウンロードしない。
- **FR-002**: 0Bまたは不完全なキャッシュは無効として扱い、Nodeが再ダウンロードを行う。
- **FR-003**: /v1/models の `ready=true` は、Node が当該モデルを利用可能と報告した場合のみ返す（フォーマットは問わない）。
- **FR-004**: /v1/chat/completions は指定モデルを必ず使用し、エンドポイント側で当該モデルがロードされること。
...
- **FR-008**: DELETE /v1/models/:name でロードバランサー登録情報が削除され、ノードのキャッシュは同期で削除される（ロードバランサーにバイナリは存在しない）。
...
- **NodeがHF等の外部ソースから直接ダウンロードしてキャッシュする**

## 成功基準 *(必須)*
1. Node のキャッシュ有/無/破損時の挙動が正しく動作すること。
