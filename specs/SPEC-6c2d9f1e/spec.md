...
### ユーザーストーリー5 - モデル削除 (P1)
運用管理者として、モデルを削除すると /v1/models から即時消え、ルーターの登録情報とノードのローカルキャッシュが削除されることを期待する（ルーターはバイナリを保持しない）。
...
- **FR-008**: DELETE /v1/models/:name でルーター登録情報が削除され、ノードのキャッシュは同期で削除される（ルーターにバイナリは存在しない）。
