...
**改定（2026-01-25）**: ロードバランサーは中央集権的なモデル管理を行わず、各エンドポイント（xLLM等）からモデル一覧を集約するのみとする。モデルのアーキテクチャ認識はエンドポイント側で行う。
...
**改定（2026-01-25）**:
1. `supported_models.json`の手動メンテナンスが運用負荷となっている
2. 新規モデル追加のたびにコード変更とリリースが必要
3. ロードバランサーがモデル認識を行う必要がない（エンドポイント側で行うべき）
...
**改定（2026-01-25）**:
- `supported_models.json`を廃止し、エンドポイント側でHuggingFace config.jsonからアーキテクチャを自動検出
- ロードバランサーは各エンドポイントの`/v1/models`を集約して返すのみ
- モデル登録はURL登録のみ（メタデータ登録）
...
### FR-1: ~~対応モデルリストの管理~~ **廃止（2026-01-25）**
...
**改定**: ロードバランサーは`supported_models.json`を持たず、各エンドポイントからモデル一覧を集約する

### FR-1-NEW: エンドポイントからのモデル集約

- ロードバランサーはオンラインエンドポイントの`/v1/models`を定期的に取得
- 全エンドポイントのモデルを統合して`/v1/models`で返す
- モデルのアーキテクチャ認識はエンドポイント側の責務
...
- エンドポイント側の実際のファイル削除はノードのモデル同期処理に委譲
...
- ~~カスタムモデル登録機能（対応モデル以外の登録）~~ **改定**: 任意のHuggingFaceモデルを登録可能に変更
- モデルの自動更新機能
- モデルバージョン管理
- ロードバランサー主導のモデルダウンロード/変換
- **新規**: ロードバランサー側でのモデルアーキテクチャ認識（エンドポイント側で行う）
- **新規**: `supported_models.json`による中央集権的なモデル定義
