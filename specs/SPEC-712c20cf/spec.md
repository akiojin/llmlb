...
WebブラウザからアクセスできるリアルタイムダッシュボードUI。ノードの状態、リクエスト処理状況、パフォーマンスメトリクスを可視化し、`/v1/chat/completions` への動作確認を行うチャットコンソール（Playground）を備える。
...
4. **前提** チャットフォームにメッセージを入力済み、**実行** Enter送信、**結果** `/v1/chat/completions` にPOSTされ、レスポンスが履歴に追記される（ストリーミングONの場合は逐次追記される）
...
- エンドポイント: `GET /playground`（UIとして提供）
- モデル選択: プロバイダー切替（`ローカル` / `クラウド` / `すべて`）でモデルリストをフィルタリング。
- 送信: `/v1/chat/completions` に対し、システムプロンプト付与の有無とストリーミングON/OFFを切り替えて送信できる。
...
- チャットコンソール: /playgroundで全画面オーバーレイ（FR-026で明記）
