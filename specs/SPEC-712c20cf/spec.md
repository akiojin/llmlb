...
ダッシュボードのデザインテーマを切り替えられる。

- 提供テーマ（7種類）:
  - Monochrome（モノクロ、ミニマル）**← デフォルト**
  - Cyberpunk（ネオンシアン/マゼンタ、グリッド、スキャンライン）
  - Retro Terminal（緑/黒、CRT効果、フリッカー）
  - Synthwave（80年代レトロフューチャー、ピンク/パープル、パルスアニメーション）
  - Ocean Deep（深海ブルー、波状背景アニメーション）
  - Ember（溶岩オレンジ、フリッカー効果、ホットスポット）
  - Forest（森林グリーン、スウェイアニメーション）
- テーマ固有の視覚効果:
  - 各テーマは色だけでなく、固有のアニメーション・効果を持つ
  - Synthwave: パルスグロー、地平線グラデーション
  - Ocean: 波状背景移動、深度ドロップシャドウ
  - Ember: 炎のフリッカー、ホットスポットグラデーション
  - Forest: 微細スウェイ、透光パッチ
  - Mono: 最小限の効果、高コントラスト
- テーマ切り替え:
  - ヘッダーにテーマ切り替えボタン配置
