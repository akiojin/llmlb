...
- **ステータス**: ⏳ 進行中
- **進捗ログ (2025-11-02)**:
  - `coordinator/tests/contract/test_proxy_chat.rs` を更新し、スタブエージェントを実ポートで起動して `/api/chat` 正常系・404系の両ケースを検証
  - `coordinator/tests/contract/test_proxy_generate.rs` を新規整備し、`/api/generate` のハッピーパスとUpstreamエラー伝搬をテスト
