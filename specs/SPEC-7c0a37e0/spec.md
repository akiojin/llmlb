...
1. **前提** 管理者としてダッシュボードにログイン済み、
   **実行** APIキー発行画面で`node`スコープを選択してキーを発行、
   **結果** 発行されたAPIキーに`node`スコープが紐付く
2. **前提** 管理者としてダッシュボードにログイン済み、
   **実行** APIキー発行画面で複数スコープ（`node`, `api`）を選択、
...
**独立テスト**: `node`スコープを持つAPIキーでノードを起動し、
ルーターへの登録が成功することを確認。権限のないキーでは拒否されることを確認。

**受け入れシナリオ**:

1. **前提** `node`スコープ付きAPIキーを所持、
...
4. **前提** `node`スコープ付きAPIキーとノードトークンを所持、
...
- **FR-002**: スコープは`node`、`api`、`admin`の3種類
- **FR-003**: ノード登録エンドポイント（`/v0/nodes`）は`node`スコープを要求する
...
- **スコープ**: APIキーに紐付く権限。`node`、`api`、`admin`のいずれか
