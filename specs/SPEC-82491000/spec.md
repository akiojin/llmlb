...
- **APIキー管理**: ダッシュボード設定画面でプロバイダー別にAPIキーを設定
- **対応プロバイダー**: 3社固定（OpenAI、Google、Anthropic）。将来的な拡張は別SPECで対応
- **コスト情報**: 静的定義（各モデルの価格はコード内またはJSONで事前定義）
- **フィルタリング**: 不要（全モデルを表示、フィルタ機能はスコープ外）
- **ルーティング優先順位**: ローカルモデル優先（同名モデルがある場合）

**実装時の判断に委ねる事項**:

- 静的コスト情報の更新頻度（推奨: リリースごと）
- ダッシュボードでのAPIキー表示形式（推奨: マスク表示 `sk-****xxxx`）
...
ダッシュボードのModelsセクションにプロバイダー別タブを追加する。

**表示条件**:

- 各プロバイダータブはAPIキーが設定されている場合（=モデルが存在する場合）のみ表示される
- Localタブにはクラウドモデルを表示しない（`owned_by === "router"` のみ）

**タブ構成**:

- 「Local」タブ: ローカルモデル一覧（常時表示）
- 「Hub」タブ: 対応モデル一覧（常時表示）
- 「OpenAI」タブ: OpenAIモデル一覧（`owned_by === "openai"` のモデルが存在する場合のみ）
- 「Anthropic」タブ: Anthropicモデル一覧（`owned_by === "anthropic"` のモデルが存在する場合のみ）
- 「Google」タブ: Geminiモデル一覧（`owned_by === "google"` のモデルが存在する場合のみ）

**フィルタリング実装**:

フロントエンド側で `/v1/models` レスポンスの `owned_by` フィールドを使用してフィルタリングを行う。
バックエンドAPIの変更は不要。

### FR-008: APIキー設定

クラウドプロバイダーのAPIキーは以下の2つの方法で設定可能とする。

**設定方法**:

1. **ダッシュボードUI**: 設定画面でプロバイダー別にAPIキーを入力・保存
   - 入力済みキーはマスク表示（`sk-****xxxx`）
   - 削除・更新が可能
   - 保存先はロードバランサーのDB（暗号化保存）

2. **環境変数**: 起動時に環境変数から読み込み
...
- ダッシュボードで設定されたキー > 環境変数
- 両方設定されている場合、ダッシュボード設定を優先

**関連仕様**:

- SPEC-d4eb8796（認証・アクセス制御）: 設定画面へのアクセス制御
- SPEC-6cd7f960（対応モデルリスト）: プロバイダー別タブ表示

### Session 2026-01-02 インタビュー（追加）

**APIキー管理**:

- APIキー検証: 保存時検証なし（使用時エラーで通知）
- APIキーセキュリティ: バックエンド経由（ブラウザに露出しない）

**同名モデル処理**:

- ローカルとクラウドで同名モデル: エラー（プレフィックス必須）
- 曖昧なリクエスト（プレフィックスなし）は拒否

**レートリミット**:

- クラウドプロバイダーのレートリミット状況: 常時表示
- 残りクォータ、リセット時刻を表示

**ヘルスチェック**:

- 詳細診断表示（レイテンシ、エラー率、スループット）
- コンポーネント別ステータス（ロードバランサー/ノード/DB/クラウド接続）
