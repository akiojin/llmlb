...
4. **前提** 4.5GB未満のGPUメモリを持つノードが登録される、**実行** ノード登録APIを呼び出す、**結果** gpt-oss-1bモデルのダウンロードが自動的に開始され、完了する
...
- ダウンロード中にノードがオフラインになった場合、どうなるか？
  - エンドポイント側でダウンロードが中断され、次回オンライン時に再開できる
...
- 同時に複数のモデルダウンロードが要求された場合、どうなるか？
  - エンドポイント側でキューイングし、順次ダウンロードを実行する（同時ダウンロード数は設定可能）
...
- **FR-009**: システムは既存のノード主導モデル同期（`GET /v1/models` + manifest参照）と整合する必要がある
...
- モデルダウンロードはエンドポイント側のディスク容量に依存する
- 進捗情報のリアルタイム更新には、ノードとロードバランサー間の安定したネットワーク接続が必要
...
- エンドポイント側にLLM runtimeがインストールされている、または自動インストール機能が有効である
- ロードバランサーとノード間の通信が正常に機能している
...
ロードバランサーが公式にサポートするモデルは次の4件に限定する。
...
モデルIDはファイル名/リポジトリ名ベースの形式を採用する（`name:tag` 形式は廃止）。
...
4. **サイズ情報なしの場合**: ファイル名ベースでIDを生成
   - 例: `mistral-small.gguf` → `mistral-small`
...
- ロードバランサーはノードの「OpenAI互換API（単一ポート、標準は LLM runtime ポート+1）」のみと通信する。  
  - 登録ヘルスチェック、モデル自動配布指示、推論プロキシ（chat/completions/embeddings）はすべてノードAPI経由。  
  - ロードバランサーがノード内部の LLM runtime を直接叩かないこと。  
- ノードは受けた OpenAI互換リクエストを内部の LLM runtime にルーティングし、必要なモデルを ensure/pull してから実行する。  
- 複数 LLM runtime 実体を持つ場合の振り分けはエンドポイント側の責務とし、ロードバランサーは単一 API エンドポイントだけを認識すればよい。  
- `/v1/models` 応答はロードバランサーが定義する対応モデル固定リストを返す（エンドポイント側で揃える）。

### モデルダウンロード完了までの受付制御
- 対応モデル4件のダウンロードがすべて完了するまではノードは `initializing` 状態とし、ロードバランサの選択対象外とする。
- ダウンロード完了後は `ready` 状態となり、リクエスト受付可能となる（モデルはオンデマンドでロードされる）。
- ロードバランサーからの推論リクエストは、全ノードが `initializing` の場合は 503（準備中）を返す。
