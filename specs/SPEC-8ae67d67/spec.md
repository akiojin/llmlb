...
> NOTE: 本仕様は採用されません。ルーターからノードへの push 配布は実装しない方針です。
> ノードはルーターの `/v1/models` と `/v0/models/blob/:model_name` を参照して自律的にモデルを取得します。
...
- **FR-009**: システムは既存のノード主導モデル同期（`GET /v1/models` + `GET /v0/models/blob/:model_name`）と整合する必要がある
...
- **廃止理由**: ルーターからノードへのpush配布は実装しない方針に変更
- **置換先**: SPEC-dcaeaec4（モデルカタログ統合）
- **新方針**: ノードはルーターの `/v1/models` と `/v0/models/blob/:model_name` を参照して自律的にモデルを取得するpull型設計に変更
