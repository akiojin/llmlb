...
```bash
REGISTER_RES=$(curl -sS http://localhost:32768/api/nodes \
...
```bash
LOGIN_RES=$(curl -sS http://localhost:32768/api/auth/login \
  -H "Content-Type: application/json" \
  -d '{
    "username": "admin",
    "password": "test"
...
curl -sS http://localhost:32768/api/nodes/${NODE_ID}/approve \
  -H "Content-Type: application/json" \
...
```bash
curl -sS http://localhost:32768/api/nodes | jq .
```

## ヘルスチェック送信（curlで確認）

`POST /api/nodes` のレスポンスに含まれる `runtime_token` を使って `POST /api/health` を呼び出します。

```bash
NODE_ID=$(echo "$REGISTER_RES" | jq -r .runtime_id)
NODE_TOKEN=$(echo "$REGISTER_RES" | jq -r .runtime_token)

curl -sS http://localhost:32768/api/health \
  -H "Content-Type: application/json" \
