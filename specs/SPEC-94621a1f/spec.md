...
7. **前提** CLI からノードを起動する、**実行** `LLM_ROUTER_URL=http://router:8080 ./llm-node` のように環境変数を指定する、**結果** 保存済み設定より優先して該当URLで登録が行われる
...
- ノードのエントリーポイントは `LLM_ROUTER_URL`, `LLM_NODE_PORT`, `LLM_NODE_HEARTBEAT_SECS` などの環境変数で設定を上書きできる。
- 同じ設定項目はローカル設定ファイル（デフォルト: `~/.llm-router/config.json`、または `LLM_NODE_CONFIG`）からも読み込まれ、環境変数が未指定のときに適用される。
...
- 現バージョンでは登録/一覧は認証なし（プライベートネットワーク前提）
