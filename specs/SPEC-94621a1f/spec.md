...
**後継SPEC**: SPEC-e8e9326e（ロードバランサー主導エンドポイント登録システム）

> **⚠️ 廃止予定**
>
> このSPECは **SPEC-e8e9326e（ロードバランサー主導エンドポイント登録システム）** により廃止されます。
>
> **廃止理由**:
>
> - ノード主導の自己登録からロードバランサー主導のエンドポイント登録に移行
...
各マシンでノードアプリケーションを起動し、ロードバランサーに自動的に登録される機能。
ノードはロードバランサーとの接続状態を管理し、ハートビートを送信する。
...
ユーザーは各マシンでノードアプリケーションを起動し、ロードバランサーに自動的に登録される。Windows / macOS 環境ではノードがシステムトレイに常駐し、ロードバランサーとの接続状態をリアルタイムで表示する。Linux では従来通り CLI 常駐プロセスとして動作する。

**この優先度の理由**: ノード登録はシステムの基盤機能であり、これがなければ他のすべての機能が動作しない。最小限の機能で動作確認が可能。

**独立テスト**: ノードを起動してロードバランサーのURLを設定し、登録が成功することで完全にテスト可能。ロードバランサー側で登録されたノード一覧を確認できれば価値を提供する。

**受け入れシナリオ**:

1. **前提** ノードアプリがインストールされていない、**実行** ノードをインストールして起動する、**結果** システムトレイにアイコンが表示され、設定ウィンドウが自動的に開く
2. **前提** ノードが起動している、**実行** 設定ウィンドウでロードバランサーのURL（例: `http://192.168.1.10:32768`）を入力して「接続」をクリックする、**結果** ノードがロードバランサーに登録され、接続状態が「接続済み」と表示される
3. **前提** ノードが登録されている、**実行** ロードバランサーのダッシュボードを開く、**結果** 登録されたノードが「pending」として一覧に表示され、マシン名、IPアドレス、LLM runtimeのバージョンが確認できる
4. **前提** 管理者がダッシュボードにログインしている、**実行** pending のノードを承認する、**結果** ノードが Registering/Online に遷移し、運用対象になる
5. **前提** ノードが登録されている、**実行** ノードアプリを終了する、**結果** ロードバランサーのダッシュボードで該当ノードが「オフライン」状態になる
6. **前提** ロードバランサーをホストするマシンである、**実行** そのマシンでノードを起動する、**結果** 同一マシン上のロードバランサーに正常に登録され、localhostとして認識される
7. **前提** WindowsまたはmacOS上でノードを起動している、**実行** システムトレイアイコンをダブルクリックする、**結果** ローカル設定パネルがブラウザで開き、ロードバランサーURLやポートなどを入力・保存できる（保存内容は次回起動時に反映）
8. **前提** CLI からノードを起動する、**実行** `LLMLB_URL=http://lb:32768 ./xllm` のように環境変数を指定する、**結果** 保存済み設定より優先して該当URLで登録が行われる
...
ロードバランサーは登録されたノード情報を永続化し、再起動後も情報を保持する。
...
- トレイアイコンのダブルクリック、またはコンテキストメニューの「設定パネルを開く」を選択すると、ローカルでホストしている設定パネル (`http://127.0.0.1:<dynamic>/`) が開き、ロードバランサーURLや監視間隔を編集できる。
- 「Dashboardを開く」から `LLMLB_URL/dashboard` に遷移でき、コンテキストメニューの「ノードを終了」で常駐プロセスを停止できる。
- Linux など非対応プラットフォームでは従来のCLI常駐プロセス（トレイなし）として動作する。

### FR-006: 環境変数／設定パネルによる上書き

- ノードのエントリーポイントは `LLMLB_URL`, `XLLM_PORT`, `XLLM_HEARTBEAT_SECS` などの環境変数で設定を上書きできる。
- 同じ設定項目はローカル設定ファイル（デフォルト: `~/.llmlb/config.json`、または `XLLM_CONFIG`）からも読み込まれ、環境変数が未指定のときに適用される。
...
- ノード→ロードバランサーのヘルスチェックは `X-Node-Token` による認証を必須とする
...
- LLMLB_URL, XLLM_PORT, XLLM_HEARTBEAT_SECS
...
- **完全独立動作可能**: ノードはロードバランサーなしでも単独で動作可能
  - ローカルAPIエンドポイントで直接推論リクエストを受付
  - モデル管理もローカルで完結
- **オフライン時の挙動**: ロードバランサーとの接続が切断されても推論機能は継続
  - ルーティング対象外になるのみ
