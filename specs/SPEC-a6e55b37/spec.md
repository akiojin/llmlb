...
運用者として、macOS/Windows/Linux の違いや、`.pkg/setup.exe` インストールと `tar.gz/zip` 展開の違いを意識せずに、可能な範囲で自動更新してほしい。Ollama のように OS ごとに更新方法が変わることは許容する。
...
3. **前提** Windows Inno Setup（`%LOCALAPPDATA%\Programs\llmlb`）実行中、**実行** 更新承認、**結果** `-setup.exe` をサイレント実行して更新→再起動する（UAC不要）
