...
- [ ] I-1: Header.tsx ghost→outline置換（3箇所）
  - 依存: C-8
  - Refresh Button, Theme Toggle, User Menu Trigger

- [ ] I-2: EndpointTable.tsx ghost→outline置換（4箇所） [P]
  - 依存: C-8
  - Details, Test, Sync, Delete アイコンボタン

- [ ] I-3: RequestHistoryTable.tsx ghost→outline置換（2箇所） [P]
  - 依存: C-8

- [ ] I-4: ApiKeyModal.tsx ghost→outline置換（3箇所） [P]
  - 依存: C-8

- [ ] I-5: InvitationModal.tsx ghost→outline置換（1箇所） [P]
  - 依存: C-8

- [ ] I-6: UserModal.tsx ghost→outline置換（2箇所） [P]
  - 依存: C-8

- [ ] I-7: EndpointPlayground.tsx ghost→outline置換（2箇所） [P]
  - 依存: C-8

- [ ] I-8: LoadBalancerPlayground.tsx ghost→outline/default置換（4箇所）
  - 依存: C-8
  - モード切替ボタン: 非アクティブ時ghost→outline
  - Settings: ghost→outline
  - Copy cURL: ghost→outline

- [ ] I-9: Dashboard.tsx 生button→Buttonコンポーネント化（2箇所）
  - "Restart to update": `<button>` → `<Button>`
  - "Try again": `<button>` → `<Button variant="link">`

- [ ] I-10: EndpointPlayground.tsx 生button→Buttonコンポーネント化（2箇所） [P]
  - 添付ファイル削除ボタン（画像・音声）

- [ ] I-11: LoadBalancerPlayground.tsx 生button→Buttonコンポーネント化（1箇所） [P]
  - 添付ファイル削除ボタン
...
- [ ] P-3: cargo test 実行・全テスト通過確認

- [ ] P-4: make quality-checks 全体検証
