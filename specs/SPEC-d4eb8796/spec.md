...
**入力**: ユーザー説明: "ルーターに認証機能を追加し、APIアクセスを制御可能にする"

## ユーザーシナリオ＆テスト *(必須)*

### ユーザーストーリー1 - 管理者による初期セットアップとログイン (優先度: P1)

システム管理者は、ルーターを初めて起動する際に、自分の管理者アカウントを作成し、
...
1. **前提** ルーターが初回起動される、**実行** 管理者がユーザー名とパスワードを設定する、
...
ルーターのAI機能を利用できるように、APIキーを発行したい。
発行したAPIキーは、必要に応じて無効化できる。

**この優先度の理由**: 外部統合はルーターの主要なユースケースであり、
...
ノードがルーターに登録する際に、専用の通信トークンを受け取り、
以降の通信（ヘルスチェック、メトリクス送信等）でそのトークンを使用することで、
なりすましを防止したい。

**この優先度の理由**: ノードとルーター間の通信が無防備だと、
...
- **localhost接続**: 認証なし（ノードがルーターと同一マシンで動作する場合）
...
- **ノードトークン**: ノードがルーターと安全に通信するための認証情報。
