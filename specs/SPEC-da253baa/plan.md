...
**機能ID**: `SPEC-da253baa` | **日付**: 2025-12-26 | **仕様**: [spec.md](./spec.md)
**入力**: `/specs/SPEC-da253baa/spec.md` の機能仕様
...
## 安定化計画（Queue Overflow テスト）

- **目的**: キュー満杯の判定をタイミング依存にせず、テストで確実に再現する
- **方針**:
  - 先行リクエストが LoadManager に「in-flight」として反映された後に次リクエストを送る
  - テスト用スタブにバリア（Notify）を設け、処理継続/解放を明示制御
  - 可能なら LoadManager の状態確認ヘルパー（テスト専用）を用意し、busy判定の可視化を行う
