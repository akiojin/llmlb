...
仕様を精査した結果、重大な曖昧さは検出されませんでした。検証結果も記載済みです。

**確認済み事項**:

- ブロック対象: git checkout, git switch, git worktree, git branch -d/-D/-m/-M（FR-002, FR-003で明記）
- 許可操作: git branch（読み取り専用）、Worktree内cd（FR-003, FR-005で明記）
- レスポンス形式: JSON（decision, reason, stopReason）（FR-007で明記）
- 複合コマンド: &&, ||, ;, |で連結された場合は全体ブロック（FR-009で明記）
...
- Claude Code PreToolUse Hook API準拠
- Bash 4.0以降、jq必須、Python3オプション
