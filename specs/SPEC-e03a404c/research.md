...
**ロードバランサー側でBase64デコード → エンドポイント側でリサイズ・正規化** を採用

### 理由

- ロードバランサーでデコードすることで、バリデーションとエラー検出を早期に実行
- エンドポイント側でモデル固有のリサイズ処理を実行（モデルによる最適解像度が異なる）
...
| 方式 | 長所 | 短所 | 判定 |
|------|------|------|------|
| ロードバランサーで完全処理 | エラー検出早い | モデル依存処理が困難 | × |
