...
運用者として、エンドポイントを登録する際に、そのエンドポイントがxLLM、LM Studio、Ollama、vLLM、またはその他のOpenAI互換APIのいずれであるかを自動的に判別してほしい。これにより、手動でタイプを指定する手間を省き、誤設定を防止できる。
...
3. **前提** LM Studioサーバーが稼働している、**実行** そのURLでエンドポイントを登録する、**結果** タイプが「LM Studio」として判別・保存される
4. **前提** vLLMサーバーが稼働している、**実行** そのURLでエンドポイントを登録する、**結果** タイプが「vLLM」として判別・保存される
5. **前提** その他のOpenAI互換APIサーバーが稼働している、**実行** そのURLでエンドポイントを登録する、**結果** タイプが「OpenAI互換」として判別・保存される
...
1. **前提** 複数タイプのエンドポイントが登録されている、**実行** ダッシュボードのエンドポイント一覧を表示する、**結果** 各エンドポイントにタイプ（xLLM/LM Studio/Ollama/vLLM/OpenAI互換）が表示される
...
- 複数の判別条件に一致する場合、より具体的なタイプ（xLLM > LM Studio > Ollama > vLLM > OpenAI互換の順）を優先する
...
- **FR-004**: システムはエンドポイントの稼働状況を定期的にチェックする必要がある（GET /v1/models）。ただし `xLLM` の場合は `/api/health` を優先し、失敗時のみ `/v1/models` にフォールバックする。
...
- **FR-021**: システムはxLLM、LM Studio、Ollama、vLLM、OpenAI互換の5タイプを判別できる必要がある
...
- **FR-031**: システムはエンドポイント一覧の `TPS` 列を昇順/降順でクライアント側ソートできる必要がある。`aggregate_tps = null`（未計測）は昇順/降順のどちらでも常に末尾に配置する
- **FR-032**: システムは `/api/tags` が `200` かつ `{"error": ...}` を返す場合、Ollamaとして判別してはならない
- **FR-033**: システムは LM Studio 判別時に `/api/v1/models` の `models[]/data[]` 形（`publisher` + `architecture/arch` + `max_context_length/loaded_instances/key/display_name/format`）を最優先根拠として扱う必要がある
- **FR-034**: システムはモデル同期時にエンドポイントタイプを引き渡し、xLLM/Ollama/LM Studio の `max_tokens` 抽出を有効化する必要がある
- **FR-035**: システムは起動時タイプ再判別後、オンラインエンドポイントのモデル同期を自動実行し、`max_tokens` をバックフィルできる必要がある
...
- **エンドポイントタイプ**: エンドポイントの種別（xLLM/LM Studio/Ollama/vLLM/OpenAI互換/不明）。判別ロジックと対応機能の紐付けに使用
...
7. xLLM/LM Studio/Ollama/vLLMの各エンドポイントが正しいタイプとして判別される成功率が95%以上
