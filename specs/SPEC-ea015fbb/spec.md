...
LLM Load Balancer Web UIを構成する全画面の一覧、各画面の目的、画面間の遷移関係を定義する。
...
| SCR-011 | `/dashboard/#playground/:endpointId` | Endpoint Playground | 特定エンドポイント向けチャットUI | SPEC-712c20cf, SPEC-5fc9fe92 |
| SCR-012 | `/dashboard/#lb-playground` | LB Playground | ロードバランサー経由のチャット/負荷試験UI | SPEC-712c20cf |
...
└─────────────────────────────────────────────────┘
     │
     ├─ Endpoint詳細「Open Playground」 ──→ SCR-011（Endpoint Playground）
     │
     └─ Header「LB Playground」 ─────────→ SCR-012（LB Playground）
```
...
1. **前提** ダッシュボードを表示中、**実行** Headerの「LB Playground」ボタンをクリック、
   **結果** `#lb-playground` 画面が開く（1クリック）
2. **前提** Endpoint詳細モーダルを表示中、**実行** 「Open Playground」をクリック、
   **結果** `#playground/:endpointId` 画面が開く（2クリック以内）
3. **前提** Playgroundを表示中、**実行** ダッシュボードに戻るボタンをクリック、
   **結果** ダッシュボードに戻る（1クリック）
4. **前提** 任意の画面を表示中、**実行** ログアウトボタンをクリック、
...
1. **前提** ダッシュボードで履歴タブを選択中、**実行** LB Playgroundに遷移して戻る、
...
- 画面数: 5画面（ログイン、登録、ダッシュボード、Endpoint Playground、LB Playground）
