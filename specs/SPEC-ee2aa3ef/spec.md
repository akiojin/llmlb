...
**依存SPEC**: SPEC-47c6f44c（自動マージ）
**入力**: ユーザー説明: "完全自動化リリースシステム"

**実行インターフェース**:
- Claude Code: `/release`, `/hotfix`
- Codex: `release` スキル, `hotfix` スキル
- 共通フォールバック: `./scripts/prepare-release.sh`, `./scripts/release/create-hotfix.sh`
...
開発リーダーとして、リリース準備が整ったらリリース開始インターフェースを一つ実行するだけで、
...
**独立テスト**: `release` 系インターフェース（Claude `/release`、Codex `release` スキル、またはスクリプト）を実行し、開発ブランチから本番ブランチへの
プルリクエストが自動作成され、品質チェック、マージ、リリース、バイナリ公開まで
一連のプロセスが完全自動で完了することを確認できる。

**受け入れシナリオ**:

1. **前提** 開発ブランチの準備完了、**実行** リリース開始インターフェース実行、
...
開発者として、本番で重大なバグを発見したらホットフィックス開始インターフェースで
...
**独立テスト**: `hotfix` 系インターフェース（Claude `/hotfix`、Codex `hotfix` スキル、またはスクリプト）を実行して修正ブランチを作成し、
修正後に本番ブランチへのプルリクエストを作成して、品質チェック合格後に
パッチバージョンでリリースされることを確認できる。

**受け入れシナリオ**:

1. **前提** 本番環境で問題を発見、**実行** ホットフィックス開始インターフェース実行、
...
- 同時に複数のリリース開始要求（Claude/Codex/スクリプト混在）が実行された場合、最初のプロセスのみが実行され、後続は待機する
...
- **FR-008**: ユーザーは単一操作（Claude `/release`、Codex `release` スキル、またはスクリプト）で正式リリースプロセスを開始できる必要がある
- **FR-009**: ユーザーは単一操作（Claude `/hotfix`、Codex `hotfix` スキル、またはスクリプト）でホットフィックスプロセスを開始できる必要がある
...
2. **リリースリーダーは単一操作（Claude/Codex/スクリプト）の実行から30分以内に正式版リリースとバイナリ公開を完了できる**
   （すべてのプラットフォームのビルド時間を含む）
3. **すべてのリリースで品質チェックの合格率が100%である**
   （不合格の場合はリリースが作成されない）
4. **リリース手順の学習時間が30分以内である**
   （新しい開発者がClaude/Codexいずれかの操作方法を理解してリリースを開始できるまで）
...
- alpha版リリース: develop→マージ時に自動作成（FR-004で明記）
- 正式版リリース: main→マージ時に自動作成 + バイナリ公開（FR-005で明記）
- バージョン計算: コミットメッセージから自動計算（FR-001で明記）
- 品質チェック: cargo fmt, clippy, test, commitlint, markdownlint（FR-002で明記）
- 対応プラットフォーム: Linux x86_64, Windows x86_64, macOS x86_64/ARM64（FR-011で明記）
- 実行インターフェース: Claudeスラッシュコマンド / Codexスキル / スクリプトの3経路で同等運用

**依存関係の確認**:

- SPEC-47c6f44c（品質チェック・自動マージ機能）に依存
