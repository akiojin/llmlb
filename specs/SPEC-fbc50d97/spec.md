...
**入力**: ユーザー説明: "ロードバランサーに送られたリクエストとノードから
...
1. **前提** ロードバランサーが複数のリクエストを処理済み、**実行** オペレーターが
...
- 7日より古いデータはどうなるか？
  - 既定では7日より古いデータを自動削除し、ユーザーには通知しない（透過的に処理）
  - 保持期間は環境変数で変更可能（`LLMLB_REQUEST_HISTORY_RETENTION_DAYS`）
...
- **FR-001**: システムはロードバランサーが受信したすべてのリクエスト（Chat、
...
- **FR-004**: システムは記録されたデータを既定7日間保持し、それより古いデータを
  自動的に削除する必要がある（保持期間は `LLMLB_REQUEST_HISTORY_RETENTION_DAYS` で変更可能）
...
- **FR-013**: システムはプライバシー設定に基づきプロンプト/レスポンス内容をマスク化して保存する必要がある（マスクなし/部分マスク/完全マスク）
...
- クリーンアップは定期実行される（既定: 3600秒、`LLMLB_REQUEST_HISTORY_CLEANUP_INTERVAL_SECS`）
...
- ロードバランサーサーバーが既に稼働しており、ノードが登録されている
...
- ロードバランサーのプロキシAPI機能（`/v1/chat/completions`, `/v1/completions`）✅ 実装済み
- ダッシュボードのバックエンドAPI機能（`llmlb/src/api/dashboard.rs`）✅ 実装済み
- ダッシュボードのフロントエンド（`llmlb/src/web/static/`）✅ 実装済み
- 既存のストレージ層実装（`llmlb/src/db/mod.rs`のパターンを踏襲）✅ 実装済み
...
- 処理: 非同期保存、プロキシに影響なし（FR-010で明記）

**エッジケースの確認**:

- ストリーミング: 完了後に全文保存（FR-012で明記）
- ストレージ不足: プロキシ継続、エラーログ記録
...
- **プロンプトマスキング**: 完全マスク対応が必要
  - デフォルトでプロンプト内容をマスク化して保存
  - 設定により「マスクなし」「部分マスク」「完全マスク」を選択可能
  - マスク対象: `messages`配列内の`content`フィールド
  - メタデータ（モデル名、処理時間、トークン数等）はマスク対象外
- **エクスポート時のマスキング**: エクスポートファイルも同様のマスキングを適用

**追加機能要件（FR-013）**:

- システムはプライバシー設定に基づきプロンプト/レスポンス内容をマスク化して保存する必要がある

**実装時の判断に委ねる事項**:

- マスク化のデフォルト設定（推奨: 完全マスク）
- 部分マスクの粒度（推奨: 先頭10文字のみ表示）

### Session 2025-12-30 (追加)

インタビューにより以下が追加確定:

- **マスク解除権限**: 管理者（admin ロール）のみがプロンプトマスキングを解除可能
  - 設定変更による一時解除（デバッグ用途）
  - 一般ユーザーはマスク済みデータのみ閲覧可能
- **クラウドプロバイダーへのリクエスト**: ローカルと同様にマスキング適用
  - クラウド（OpenAI/Anthropic/Google）へのリクエストもログ上でマスク
  - プロンプト内容は暗号化せずにクラウドに送信されるが、ログ記録時にマスク
- **デバッグモード**: 設定変更でマスキングを一時的に無効化可能（管理者権限必須）
