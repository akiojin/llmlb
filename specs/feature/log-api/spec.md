# Feature: Agent/Coordinator Log Retrieval API
...
- AC-002: 存在しないログファイルでも 200・空ボディ。  
- AC-003: コーディネーター `/api/agents/:id/logs?tail=3` が 200 を返し、エージェントからの3行がそのまま届く。  
- AC-004: エージェント停止中にアクセスすると 502 かつ `error` メッセージが含まれる。  
- AC-005: ダッシュボードのログパネルで最新ログテキストが表示される（手動E2E確認またはスモーク）。
